"""Train MLP on DeBERTa-v3 embeddings + statistical features."""
import os
import time
import numpy as np
import pandas as pd
...
OUTPUT_PATH = os.path.join(DATA_PATH, "submission_deberta.csv")
...
def main():
    ...
    # Load cached DeBERTa embeddings
    print("Loading cached DeBERTa-v3 embeddings (float16)...")
    X_train_emb = np.load(os.path.join(EMBED_PATH, "train_deberta.npy")).astype(np.float16)
    X_test_emb = np.load(os.path.join(EMBED_PATH, "test_deberta.npy")).astype(np.float16)
    ...
    # Test stats from ALL training data
    ...
    tr_numeric = np.column_stack([
        ...
        tr_user_avg, tr_user_cnt, tr_prod_avg, tr_prod_cnt, tr_parent_avg, tr_parent_cnt,
    ])
    val_numeric = np.column_stack([
        ...
        val_user_avg, val_user_cnt, val_prod_avg, val_prod_cnt, val_parent_avg, val_parent_cnt,
    ])
    test_numeric = np.column_stack([
        ...
        te_user_avg, te_user_cnt, te_prod_avg, te_prod_cnt, te_parent_avg, te_parent_cnt,
    ])
    ...
    print(f"\n=== Training MLP on DeBERTa-v3 ({EPOCHS} epochs, early stop={EARLY_STOP_PATIENCE}) ===")
    print(f"Architecture: {input_dim} → {HIDDEN1} → {HIDDEN2} → {HIDDEN3} → 1")
    print(f"Features: 768 DeBERTa + 2 numeric + 6 stat = {input_dim}")
